        ...
        : TForm(Owner)
{
	...
	sprintf(g_szstr_Version,"Winny v2.0b%g",6.6);
...
void __fastcall TMainForm::SearchButtonClick(TObject * Sender)
{
	...
	if (local_00.Length()>=2) {
		...
		bool		local_09=false;
		for (int local_10=0; local_10<local_08; local_10++) {
...
void __fastcall TMainForm::SubClosePanelButtonClick(TObject * Sender)
{
	...
	if (! (!BbsButton->Down||!(!TopButton->Down)&&BottomButton->Down) ) {
		TopButton->Down=true;
	}
	mf_TMainForm_0080();
	MainPanel->EnableAlign();
...
void __fastcall TMainForm::TabCloseClick(TObject * Sender)
{
	...
	AnsiString local_00=(TabControl->Tabs->Strings[TabControl->TabIndex]);
	for (int local_01=0; TopCheckListBox->Items->Count>local_01; local_01++) {
		if ((TopCheckListBox->Items->Strings[local_01]==local_00)) {
			TopCheckListBox->Checked[local_01]=false;
		}
	}
...
void __fastcall TMainForm::SubTabCloseClick(TObject * Sender)
{
	...
	AnsiString local_00=(SubTabControl->Tabs->Strings[SubTabControl->TabIndex]);
	for (int local_01=0; BottomCheckListBox->Items->Count>local_01; local_01++) {
		if ((BottomCheckListBox->Items->Strings[local_01]==local_00)) {
			BottomCheckListBox->Checked[local_01]=false;
		}
	}
...
void TMainForm::mf_TMainForm_0140()
{
	...
	if (BbsButton->Down) {
		...
		case 6:  
			...
			TopToolButton3->Caption="変換タスク保留";
		...
		case 6:  
			...
			BottomToolButton3->Caption="変換タスク保留";
...
void __fastcall TMainForm::BbsButtonClick(TObject * Sender)
{
	...
	if (BbsButton->Down) {
		TopButton->Down=OptionForm->InitialBbsViewTopPaneCheck->Checked;
		BottomButton->Down=OptionForm->InitialBbsViewBottomPaneCheck->Checked;
		LeftButton->Down=OptionForm->InitialBbsViewLeftPaneCheck->Checked;
	} else {
		TopButton->Down=OptionForm->InitialFileViewTopPaneCheck->Checked;
		BottomButton->Down=OptionForm->InitialFileViewBottomPaneCheck->Checked;
		LeftButton->Down=OptionForm->InitialFileViewLeftPaneCheck->Checked;
	}
	mf_TMainForm_0080();
	TabControlChange(TabControl);
	TabControlChange(SubTabControl);
...
void __fastcall TMainForm::FileIgnoreClick(TObject * Sender)
{
	...
	for (local_03=0; local_00->Items->Count>local_03; local_03++) {
		...
		g_pCMainControl->mf_CMainControl_0070(local_00->Items->Item[local_03]->Caption.c_str(),NULLPO,local_04,"",0.0,0.0,false,false,true,true);
		local_02=local_02+1;
		if (local_02>=100) break;
	}
	if (local_02!=0) {
