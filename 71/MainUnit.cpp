        ...
        : TForm(Owner)
{
	...
	sprintf(g_szstr_Version,"Winny v2.0b%g",7.1);
	...
	TransStatusBar->Height=6-TransStatusBar->Font->Height;
	mf_TMainForm_0000();
	mf_TMainForm_0160();
}
	...
	if (local_00.Length()>=2) {
		...
		for (local_10=0; local_10<local_08; local_10++) {
			...
			if (local_02.AnsiCompare(local_00)==0) {
				if (!OptionForm->KeepSearchHistoryCheck->Checked) {
					SearchComboBox->Items->Delete(local_10);
				}
				local_08=local_08-1;
				local_10=local_10-1;
				if (OptionForm->KeepSearchHistoryCheck->Checked) break;
			}
		}
		if (!OptionForm->KeepSearchHistoryCheck->Checked||local_10==local_08) {
