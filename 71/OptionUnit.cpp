...
//	int local_02;
	...
	DownloadIgnoreKeyCheck->Checked=g_pCMainControl->m_unknown_000_70;
	IgnoreSortCheck->Checked=g_pCMainControl->m_unknown_001_70;
	MakeMarkCheck->Checked=g_pCMainControl->m_optflg_makeMarkFile_;
	MaxNameLengthEdit->Text=g_pCMainControl->m_int_maxnamelen_70;
	...
	sprintf(local_03,"%X",0x80000005);
	local_00=g_pTIniFile->ReadString("Color","BbsWindowBackGround",local_03);
	sscanf(local_00.c_str(),"%X",&local_01);
	...
	sprintf(local_03,"%X",0x8000000F);
	local_00=g_pTIniFile->ReadString("Color","SearchBarBackGround",local_03);
	sscanf(local_00.c_str(),"%X",&local_01);
	SearchBarColorBox->Selected=local_01;
	sprintf(local_03,"%X",0x80000012);
	local_00=g_pTIniFile->ReadString("Color","SearchBarString",local_03);
	sscanf(local_00.c_str(),"%X",&local_01);
	...
	sprintf(local_03,"%X",0x8000000F);
	local_00=g_pTIniFile->ReadString("Color","BbsSearchBarBackGround",local_03);
	sscanf(local_00.c_str(),"%X",&local_01);
	BbsSearchBarColorBox->Selected=local_01;
	sprintf(local_03,"%X",0x80000012);
	local_00=g_pTIniFile->ReadString("Color","BbsSearchBarString",local_03);
	sscanf(local_00.c_str(),"%X",&local_01);
	...
	KeepSearchHistoryCheck->Checked=g_pTIniFile->ReadInteger("Option","KeepSearchHistory",0)>0;
	int local_02=g_pTIniFile->ReadInteger("Option","BbsInterLineSpace",2);
	MainForm->BbsView->ExternalLeading=local_02;
	BbsInterLineSpaceEdit->Text=local_02;
...
void __fastcall TOptionForm::OkButtonClick(TObject * Sender)
{
	...
	g_pCMainControl->m_unknown_000_70=DownloadIgnoreKeyCheck->Checked;
	g_pTIniFile->WriteInteger("Option","DownloadIgnoreKey",DownloadIgnoreKeyCheck->Checked?1:0);
	...
	g_pCMainControl->m_unknown_001_70=IgnoreSortCheck->Checked;
	g_pTIniFile->WriteInteger("Option","IgnoreRefSort",IgnoreSortCheck->Checked?1:0);
	g_pCMainControl->m_int_maxnamelen_70=callatol(MaxNameLengthEdit->Text.c_str());
	if (g_pCMainControl->m_int_maxnamelen_70<32) {
		g_pCMainControl->m_int_maxnamelen_70=32;
	}
	if (g_pCMainControl->m_int_maxnamelen_70>255) {
		g_pCMainControl->m_int_maxnamelen_70=255;
	}
	g_pTIniFile->WriteInteger("Option","MaxNameLength",g_pCMainControl->m_int_maxnamelen_70);
	...
	g_pTIniFile->WriteInteger("Option","KeepSearchHistory",KeepSearchHistoryCheck->Checked?1:0);
	int local_03a=callatol(BbsInterLineSpaceEdit->Text.c_str());
	if (local_03a<0) {
		local_03a=0;
	}
	if (local_03a>50) {
	...
	g_pTIniFile->WriteInteger("Option","BbsInterLineSpace",(int)local_03a);
	Close();
	...
	g_pCMainControl->mf_CMainControl_7000();
	return;
}
...
void __fastcall TOptionForm::InitButtonClick(TObject * Sender)
{
	...
	DownloadIgnoreKeyCheck->Checked=false;
	IgnoreSortCheck->Checked=true;
	MaxNameLengthEdit->Text="255";
	return;
}
